...
function isSyntacticallyCompatible(pattern) {
    ...
    let uPattern;
    try {
        uPattern = new regexpp_1.RegExpParser().parsePattern(pattern.raw, undefined, undefined, true);
    }
    catch (error) {
        return false;
    }
...
function isCompatibleCharLike(char, flags, uFlags) {
    const cs = (0, regexp_ast_analysis_1.toCharSet)(char, flags);
    if (!cs.isDisjointWith(SURROGATES)) {
        return false;
    }
    const uCs = (0, regexp_ast_analysis_1.toCharSet)(char, uFlags);
...
function isCompatibleQuantifier(q, flags, uFlags) {
    ...
    const cs = (0, regexp_ast_analysis_1.toCharSet)(q.element, flags);
    if (!cs.isSupersetOf(SURROGATES)) {
        return false;
    }
    const uCs = (0, regexp_ast_analysis_1.toCharSet)(q.element, uFlags);
