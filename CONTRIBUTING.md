...
All pull requests should be associated with issues. Although not strictly required, the pull requests should be made to [the GitLab repository issues board](https://gitlab.com/megabyte-labs/ansible-roles/[[ The `meta/main.yml` file needs to be setup. The `galaxy_info.role_name` value is missing. ]]/-/issues) instead of the [GitHub mirror repository](https://github.com/ProfessorManhattan/ansible-[[ The `meta/main.yml` file needs to be setup. The `galaxy_info.role_name` value is missing. ]]). This is because we use GitLab as our primary repository and mirror the changes to GitHub for the community.
